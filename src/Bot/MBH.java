    ...
    public List<ShootMBHAction> shootOnBestPossibleTarget(boolean isLateGamePhase) {
        ...
        if (p != null) {
            if (!isLateGamePhase || Controll.game.getSettings().getMaxConcurrentActions() <= 3)
                actions.add(Bot.MBH.sendMBH(Planets.findClosestOwnedPlanetToTarget(p).getId(), p.getId()));
            for (int i = 0; i < 2; i++)
                actions.add(Bot.MBH.sendMBH(Planets.findClosestOwnedPlanetToTarget(p).getId(), p.getId()));
            return actions;
        }
        ...
        if (isLateGamePhase) {
            ...
            actions.add(Bot.MBH.sendMBH(closestPlanet.getSecond().getFirst().getId(), closestPlanet.getSecond().getSecond().getId()));
            ...
            actions.add(Bot.MBH.sendMBH(closestUnhabitablePlanet.getSecond().getFirst().getId(), closestUnhabitablePlanet.getSecond().getSecond().getId()));
